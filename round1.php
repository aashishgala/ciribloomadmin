...
$SqlQuizzData = mysqlQuery("SELECT * from userscore WHERE user_id = '".$userid."' AND quizz_stage IN(1,-1)");
$RowQuizzData=mysqliFetch($SqlQuizzData);
// print_r($RowQuizzData);
$UserQuizStage ='';
if (empty($RowQuizzData))
{
    $SqlInsert = "INSERT INTO userscore (`user_id`, `quizz_stage`, `uniq_id`) VALUES ('".$userid."','-1','".$uniq_id."')";
...
{
    $UserQuizStage = 1;
    header("Location:./quizone.php?completed=yes");
}
// elseif (!empty($RowQuizzData) && $RowQuizzData['quizz_stage'] == '-2') 
// {
//     header("Location:./round2.php");
// }
// elseif (!empty($RowQuizzData) && in_array($RowQuizzData['quizz_stage'], array('2')))
// {
//     header("Location:./quizone.php?completed=yes");
// }
?>
...
<body>
    ...
    <!-- start Quiz button -->
    <div class="start_btn"><button>Start Quiz Round 1</button></div>
    <?php 
    $Rules = array(
