...
if (isset($_GET['completed']) && $_GET['completed'] == 'yes') {
    ?>
    <script>
    alert('You Have Complete First Round...!');
...
if (isset($_POST['userdata']) && $_POST['userdata']) {
    ...
    $sel = "SELECT * FROM userdata WHERE fname = '".$fname."' AND lname = '".$lname."' AND fathername = '".$fathername."' AND mothername = '".$mothername."' AND quizz_stage = 1 LIMIT 1";
    ...
    if(mysqli_num_rows($sql_chk_dup) > 0) {
        ...
        // if user has complete 2 nd round first then get uniq_id 
        $uniq_id = get_val("SELECT uniq_id FROM userdata WHERE fname = '".$fname."' AND lname = '".$lname."' AND fathername = '".$fathername."' AND mothername = '".$mothername."' AND quizz_stage = 2 LIMIT 1","uniq_id");
        ...
        $ins = "INSERT INTO `userdata`(`uniq_id`, `fname`, `fathername`, `mothername`, `lname`, `country`, `date`, `quizz_stage`) VALUES ('".$uniq_id."','".$fname."','".$fathername."','".$mothername."','".$lname."','".$country."','".$CurrentTime."','1')";
        $sql_ins = mysqlQuery($ins);
        if ($sql_ins) {
            $last_id = mysqli_insert_id($con);
            $uniq_id = get_val("SELECT uniq_id FROM userdata WHERE user_id='".$last_id."'","uniq_id");
